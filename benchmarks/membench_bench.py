...
def index_turns(collection, message_list, item_key: str):
    ...
    Returns (n_turns, doc_texts, sid_by_global_index) or (0, [], []).
    """
    docs, ids, metas = [], [], []
    ...
    for s_idx, session in enumerate(sessions):
        ...
        for t_idx, turn in enumerate(session):
            ...
            docs.append(text)
            ids.append(doc_id)
            resolved_sid = int(sid) if isinstance(sid, (int, float)) else global_idx
            metas.append(
                {
                    "item_key": item_key,
                    "sid": resolved_sid,
            ...
            sid_by_global.append(resolved_sid)
            global_idx += 1

    if docs:
        collection.add(documents=docs, ids=ids, metadatas=metas)
        return len(docs), docs, sid_by_global
    return 0, [], []
...
def run_membench(
    ...
    for idx, item in enumerate(items, 1):
        ...
        n_indexed, corpus_docs, sid_by_global = index_turns(collection, item["turns"], item_key)
        if n_indexed < 1:
            continue
        ...
        if mode == "hybrid":
            ranked_g = full_ranking_after_fusion(
                retrieved_global,
                list(raw_distances),
                corpus_docs,
                question,
            ...
            retrieved_global = ranked_g
            retrieved_sids = [sid_by_global[g] for g in ranked_g]
        else:
    ...
    print(f"\n{'=' * 58}")
    print(f"  RESULTS — VeriMem on MemBench ({mode} mode, top-{top_k})")
    print(f"{'=' * 58}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VeriMem × MemBench Benchmark")
